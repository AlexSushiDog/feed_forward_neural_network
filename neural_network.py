...
class neural_network: #Feed Forward Network Class
    ...
    def forward(self, x): #Function to perform forward pass through network
        ...
        return sig(np.dot(self.output_weights, hidden_state_two))
    ...
    def back(self, t_in, t_out, alpha):
        ...
        #forward pass
        hidden_state_one = np.dot(self.input_weights, t_in)
        hidden_state_one_sig = sig(hidden_state_one)
        hidden_state_two = np.dot(self.hidden_weights_one, hidden_state_one_sig)
        hidden_state_two_sig = sig(hidden_state_two)
        a_out = np.dot(self.output_weights, hidden_state_two_sig)
        a_out_sig = sig(a_out)
